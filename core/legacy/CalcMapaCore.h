...
public:
	...
	BOOL	bfProgressao,	// Se se trata de mapa que envolve progress�o
			bfArcoSolar,	// Are we doing -p0 solar arc chart? 
			bfSideral,		// Se Sideral,
			bfGeodesico,	// Se Geod�sico
			bfCusp, 
			bfUraniano,		// Se inclui ou n�o objetos transuranianos( n�o consideradas nesta vers�o )
			bfVelocidade,	
			bfHouseAngle,
			bfFlip,
			bfDecan,
	...
	double m_dObliqEcliptica;	// Obliq�idade da ecl�ptica

	double m_dOffSet;			// Sol M�dio 
	double m_dJD;				// Dia Juliano
	double m_dJDProgress;
	double m_dProgressDay;		// Dia progredido

	double m_dMC;				// Meio do C�u 
	double m_dAsc;				// Ascendente
	double m_dRA;				// Ascens�o reta
	double m_dPosVertice;

	double m_dOffSetGrauSideral;	// Offset angular para Sideral
	double m_dOffSetZodiacal;		// Offset angular para mudan�a de localidade ( deslocamento )
	double m_dHora;
	double m_dZonaTempo;
	double m_dHoraVerao;

	int m_nPassoEfemeride;	// N�mero de dias entre duas entradas na efen�ride = 80 dias
	...
	void   CalcularRetorno( int nObj, double dLongEclipticaRetorno, double dLat, double dLon, double dZona, double dHoraVerao, int* nDia,int* nMes, int* nAno, double* dHora );
