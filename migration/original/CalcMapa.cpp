...
#include "math.h"
#include "Hora.h"
#include "DiaJuliano.h"
#include "CalcMapa.h"
#include "Data.h"
#include "Float.h"

#include "ArrayEntidades.h"
...
CONST double rErrorData[72+51+42*3] =	{	
											...
											0.00198,528.1,48.6,-.0002,1000.4,-46.1				// Erros de Plutão
										};
...
CCalcMapa::CCalcMapa()
{
	...
	m_aoszObjetos.Add( "Mercúrio" );		//3		
	m_aoszObjetos.Add( "Vênus" );			//4
	m_aoszObjetos.Add( "Marte" );			//5
	m_aoszObjetos.Add( "Júpiter" );			//6
	m_aoszObjetos.Add( "Saturno" );			//7	
	m_aoszObjetos.Add( "Urano" );			//8	
	m_aoszObjetos.Add( "Netuno" );			//9
	m_aoszObjetos.Add( "Plutão" );			//10
	...
	m_aoszObjetos.Add( "Príapo" );			//28	
	m_aoszObjetos.Add( "Cauda" );			//29	
	m_aoszObjetos.Add( "Meio do Céu" );		//30

	m_aoszSignos.Add( "Áries" );
	m_aoszSignos.Add( "Touro" );
	m_aoszSignos.Add( "Gêmeos" );
	m_aoszSignos.Add( "Câncer" );
	m_aoszSignos.Add( "Leâo" );
	m_aoszSignos.Add( "Virgem" );
	m_aoszSignos.Add( "Libra" );
	m_aoszSignos.Add( "Escopião" );
	m_aoszSignos.Add( "Sagitário" );
	m_aoszSignos.Add( "Capricórnio" );
	m_aoszSignos.Add( "Aquário" );
...
// Cálculo do dia e hora para o retorno de qualquer objeto
// nObj:         Índice para objetos como definidos em defines.h: #define oEar 0 #define oSun 1 #define oMoo 2
// dLongRetorno: Valor da longitude eclíptica do objeto a ser retornado em Graus decimais
// dLat e dLon : Latitude e Longitude do local 
// *nDia       : Pointers com valor inicial data, serão atualizadas com o data que ocorrerá o retorno
// *nMes		
// *nAno
...
void CCalcMapa::CalcularRetorno( int nObj, double dLongRetorno, double dLat, double dLon, double dZona, double dHoraVerao, int* nDia,int* nMes, int* nAno, double* dHora )
{
	...
	double dValAnterior = 0.0;  // Para medir a convergência
	...
	while( 1 )
	{
		...
		{
			break;  // Não está mais convergindo, considera o valor atual como correto
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gera a tabela de Lua fora de curso para o período especificado. O período pode se estender para fora dos limites 
// especificados. O período inicia-se sempre com o último aspecto encontrado e com a última passagem de signo para signo
// A tabela consiste de um array de double com seqúência Dia Juliano inicial, Dia Juliano final, Dia Juliano inicial,....
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
							   ...
							   double dZona, double dHoraVerao,CProgressCtrl* pPGStatus )
{
	...
	while ( 1 )
	{
		...
		{
			if( AfxMessageBox( _T("Interrompe operação?"), MB_ICONQUESTION | MB_YESNO ) == IDYES )
			{
				pPGStatus->SetPos( 0 );
				ma_bfLFC = FALSE;
				return;
		...
		// Obtém a data e hora do final da LFC 
		nDia = nDiaF;
		nMes = nMesF;
		nAno = nAnoF;
		dHora = dHoraF;
		...
		// Prepara para o próximo final de período

		CHora oHn( dHora, FALSE );

		CDiaJuliano oAux( nDia, nMes, nAno, oHn );

		// Soma um dia, porque em um dia não há mudança de signo e avança o suficiente para não repetir data
		...
		// Obtém a data e hora do início da LFC 
		...
		// Trunca para minuto de precisão
		oJDi.PrecisaoMinuto();
		oJDf.PrecisaoMinuto();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Dado o dia, mes, ano e hora, calcula a data e hora em que a Lua entra no próximo signo
// Esta data coresponde ao final do período no qual a Lua está fora de curso
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double CCalcMapa::CalculaFimLuaForaDeCurso( int& nDia, int& nMes, int& nAno, double dLat, double dLon, double& dHora, double dZona, double dHoraVerao )
{
	...
	// Longitude eclíptica lunar para o próximo signo
	double dLongLua = (( (int)( (sChartPos0.LongitEcliptica[ oMoo ] / 30.0 )+1 ) ) * 30.0 ) * PI/180.0; 
	
	// Determina a data em que a Lua vai ocupar esta longitude eclíptica
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Dado o dia, mes, ano e hora do final do período em que a Lua está fora de curso, determina ( retrogradando o tempo )
// o início deste período
//
// Nota: O período em que a Lua está fora de curso(LFC) corresponde ao período que vai desde o último aspecto que a Lua formou
//       quando estava num determinado signo até a sua passagem para o signo seguinte.
//       Os Aspectos considerados são com o Sol e com os planetas: Mercúrio,Venus,Marte,Jupiter,Saturno,Urano,Netuno e Plutão
//       Os aspectos considerados são as principais: Conjunção,Oposição,Trígono,Sextil e Quadratura
//       A margem para o aspecto é de 1 minuto de arco. ( Ex: para quadratura 89 graus 59 minutos  e 30 segundos até 90 graus 
//       00 minuto e 30 segundos), o que deve dar a precisão de um minuto ( para mais e para menos ) na determinação do período. 
//       Como a posição da Lua também tem uma margem de erro de 1 minuto; a precisão final na determinação do período deve ser 
//       de 2 minutos ( para mais e para menos )
//       Como este período pode estar em qualquer tempo ( passado ou futuro ), o horário de verão não será considerado, isto deve
//       ser feito na apresentação do resultado onde deverá ser indicado se está ou não considerando o horário de verão.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CCalcMapa::CalculaInicioLuaForaDeCurso( int& nDia, int& nMes, int& nAno, double dLat, double dLon, double& dHoraMin, double dZona, double dHoraVerao )
{
	...
	// Em que signo a lua está entrando
	...
	double dDeltaT = -0.00069444445; // retrogradar 1 minuto de hora em fração do dia
	...
	while( 1 )
	{
		...
		for( int p = oSun; p < oChi; p++ )
		{
			...
			// Considerando um erro( margem ) de 1 minuto, o que deve dar à lista a precisão de mais ou menos 1 minuto

			if( ( dDif >= 359.9916667 || dDif <= 0.0083333 )   ||  // Se existe Conjunção
				( dDif >= 59.9916667 && dDif <= 60.0083333 )   ||  // Se existe Sextil
				( dDif >= 89.9916667 && dDif <= 90.0083333 )   ||  // Se existe Quadratura
				( dDif >= 119.9916667 && dDif <= 120.0083333 ) ||  // Se existe Trígono
				( dDif >= 179.9916667 && dDif <= 180.0083333 ) )   // Se existe Oposição
			{
				bTem = TRUE;
				break;
			}
								...
								double dHoraVerao, CArrayEntidades* paEntidades )
{
	...
	// Tratamento do tempo de construção do mapa
	...
	CalcularAstros();							// Calcular os planetas, asteróides, Lua e seu Nodo 
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Define a posição de Príapo
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if( m_bCalcFs )
	{
		// Príapo é em função de Black Moon ( Lilith )
	...
	if( ma_nHarmonicos > 1 )    // Mapa com harmônicos?
	{
		for( i = 0; i <= cObj; i++ )
		{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Transforma as coordenadas eclípticas ( Latitude e Longitude ) para obter as 
	// equatoriais ( Ascensão Reta e Declinação
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	for( i = 0; i <= cObj; i++ ) 
	{
		if( !nIgnorar[i] ) 
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Modifica os valores básicos de acordo com os flags
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	// Arco solar ou mapa de progressão? 
	...
	*/
	...
	// Verificar se algum objeto esta sendo forçado a possuir algum valor em particular
	for( i = 0; i <= cObj; i++ )
	{
		if( m_adForca[i] != 0.0 ) 
		{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Se vai executar deslocamento de posições
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if( bfFlip) 
	{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Cálculo do Ângulos das Linhas de divisão entre os signos para o desenho do mapa
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	// sChartPos0.cusp[1] contém o ângulo da cúspide da casa I contada a apartir de 0 graus de áries 
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Cálculo dos ângulos da cúspides para desenho
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	dShiftedRad = PI;
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Cálculo dos ângulos dos objetos( Planeta, Lua, ânulos, etc.) para desenho
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	for( i=0; i < 31; i++ )
	{		
...
BOOL CCalcMapa::VerificaData()
{
	...
	{
		AfxMessageBox( "Data fora do intervalo!\nEfeméride válida para 01 de Janeiro de 5200 A.C até 31 de Dezembro de 3225 D.C ");
...
{
	//////////////////////////////////////////////////////////////////////////////////////////////
	// Abre todos os arquivos de efemérides 
	//////////////////////////////////////////////////////////////////////////////////////////////
...
void CCalcMapa::SetValoresIniciais()
{
	...
	// Fazer os ajustes de acordo com as restrições
	...
	for( int i = 0; i <= cObj; i++ ) 
	{
		...
		sChartPos0.LongitEcliptica[i] = sChartPos0.LatitEcliptica[i] = 0.0;	// Coordenadas Eclípticas
		sChartPos0.Velocidade[i] = 1.0;										// 1 Direto, -1 Retrógrado
	}
...
void CCalcMapa::CalcularCasas( BOOL bDetermineData )
{
	...
	if( !ma_bfRetorno )
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// Cálculo do ascendente e Meio do Céu
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		int nSistema = 8;	// Topocêntrico ( Fixo, nesta versão não tem como mudar )

		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// Calcular as cúspides das casas
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{	
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Computar posições dos Planetas
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	ComputarPlanetas();

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Computar posição da Lua
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Certos Objetos necessitam ser calculadas com maior precisão
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if( m_bCalcFs )
	{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Computar posições dos Planetas, Sol, Lua, os 4 Asteróides
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CCalcMapa::ComputarPlanetas()  
{
	...
	while( index <= ( bfUraniano ? oNorm : cPlanet) )	// bfUraniano = FALSE e oNorm = uranHi = 40;
	{
		...
		INQ = ValorToCoefQuad( pOrbElem->in0, pOrbElem->in1, pOrbElem->in2 ); // Inclinação
		...
		helioz[index] = G;												// Coordenada Hélio ecliptica retangtular 
	...
	// Uma segunda passada é necessário para Mapa Geocêntricos ou com corpo central que não seja o Sol 
	// Por exemplo. não podemos achar a posição de Mercúrio em relação ao Plutão até que a posição de 
	// Plutão em relação ao Sol seja conhecida, e como Mercúrio foi calculado antes, necessitamos de outra passada
...
// a posição zodiacal e declinação
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CCalcMapa::ProcessarPlanetas( int index, double dAberracao )
{
	...
	while( ang > DEGQUAD )    // Assegura que delinação esté entre -90..+90 degrees
	{
		ang -= DEGHALF;
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Calcula a posição e declinação da Lua e nodo norte da Lua. 
///////////////////////////////////////////////////////////////////////////////////////////////
void CCalcMapa::ComputarLua( double* moonlo, double* moonla, double* nodelo, double* nodela )
{
	...
	LL = 973563.0+1732564379.0*m_dEscTempo-4.0*T2;	// Longitude lunar média */

	G  = 1012395.0+6189.0*m_dEscTempo;				// Longitude média do Sol no perigeu */

	N  = 933060.0-6962911.0*m_dEscTempo+7.5*T2;		// Nodo lunar méedio */

	G1 = 1203586.0+14648523.0*m_dEscTempo-37.0*T2;	// Longitude média do perigeu lunar */

	D  = 1262655.0+1602961611.0*m_dEscTempo-5.0*T2;	// Elongação média da Lua a partir do Sol */

	L  = ( LL-G1 )/M;								// Ângulos auxiliares
	L1 = ( ( LL-D)-G)/M;				
	...
	// Cálculo da perturbações da Lua
	...
	*moonla = MB = Sinal( MB ) * ( ( fabs( MB)/M) / DEGMAX - floor( ( fabs( MB ) / M ) / DEGMAX ) ) * DEGMAX;
	...
	// Cômputo do nodo lunar, Falso ou verdadeiro
	if( bfNodoVerdadeiro )
	{
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Calcular a posição da Parte da Fortuna
///////////////////////////////////////////////////////////////////////////////////////////////
...
double CCalcMapa::ProcessaEntrada( BOOL bDetermineData )
{
	...
	// Assegurar que  o mapa nâo esteja sendo feito exatamente( ? ) para os polos Norte e Sul
	...
	if( bDetermineData ) 
	{
		...
		{
			// Determina a data na qual o mapa progredido está sendo calculado
			m_dEscTempo = ( ( m_dJD + m_dHora/24.0 + ( m_dJDProgress - ( m_dJD + m_dHora/24.0) ) / m_dProgressDay) - 2415020.5) / 36525.0;
		}
	}

	// Cálculo do valor da obliqüidade da eclíptica para o valor da escala de tempo( Data ) considerada
	m_dObliqEcliptica = DEGTORAD( 23.452294-0.0130125 * m_dEscTempo );

	dNodoLunarMedio = Mod( ( 933060-6962911*m_dEscTempo+7.5*m_dEscTempo*m_dEscTempo )/3600.0 );  // Nodo lunar médio

	dSolMedio = ( 259205536.0*m_dEscTempo+2013816.0)/3600.0;									// Sol médio
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Calcula algumas variáveis utilizadas na construção do mapa
///////////////////////////////////////////////////////////////////////////////////////////////
void CCalcMapa::ComputarVariaveis()
{
	...
	m_dMC = Mod( m_dOffSetGrauSideral + RADTODEG( A ) );				// Meio do céu
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Cúspide do Ponto Este ( nome correto: Ascendente equatorial )
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Calcula a posição das cúspides de acorco com o sistema adotado
// Nesta versâo somente a Topocêntrica é considerada
///////////////////////////////////////////////////////////////////////////////////////////////
void CCalcMapa::ComputarCasas( int nSistema )
{
	...
	{
		osz.Format( "O sistema %s não foi definido para ser uilizadao em latitudes extremas", m_aoszSistema.GetAt( nSistema ) ); 
	...
	// Move o Ascendente se cair na metade inválida do zodíaco
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Cálculo dos ângulos das cúspides no sistema topocêntrico
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Dada uma posição zodial, retorna a casa em que cai.
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Dado um ângulo zodial, transforma-o em signi Decano., onde cada signo é trisecionado em três
// signos de seu elemento. Por exemplo: 1 Àries -> 3 Áries, 10 Leâo -> 0 Sagitário, 25 
// Sagitário -> 15 Leâo
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Dado uma posição zodiacal, transforma-o em equivalente Navamsa
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Dado o dia, o mês e o ano, converte no valor correspondente ao dia Juliano
// isto é, num número de dias decorrido desde uma referência fixa
///////////////////////////////////////////////////////////////////////////////////////////////
long CCalcMapa::DiaMesAnoToJuliano( int dia, int mes, int ano )
{
	if( m_bCalcFs )
	{
		...
		if( j > 2299171)                  // Trata as datas no calendário Gregoriano
		{
...
// ReversoJuliano é o inverso de DiasJulianosAbsolutos()
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Esta rotina retorna o número de dias Juliano Absolutos para uma data no calendário
// Se gregflag == 1 -> Assume-se que é o calendário Gregoriano
// Se gregflag == 0 -> Assume-se que é o calendário Juliano
// Obs: Não confundir Dia Juliano com calendário Julinao ( nem é a mesma pessoa )
...
CString CCalcMapa::DecToHms( double dDec ) 
{
	...
	// Os segundos contidos em dValor podem estar muito próximos de 1 minuto.
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Retorna a menor distância angular entre dois graus do zodíaco. Normalmente 
// é a diferença entre eles mas cuidados especiais devem ser tomados em posiçõs
// próximas a Áries
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Mesmo que a rotina MinimaDistancia() com diferença que esta rotina retorna valor positivo
// ou negativo dependendo da posição entre os dois graus
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Dadas as coordenadas x e y, retorna o ângulo formado pela linha que parte da origem
// até estas coordenadas
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Função de módulo, normaliza o parâmetro para intervalo 0 e 360 graus
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Divisão que arredonda para baixo
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Módulo para normaloizar entre 0 e 2 PIs radianos
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Módulo, normaliza inteiro entre 1 e 12
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Dados tres valores, retorna os coeficientes de uma equação quadrática em 
// função da escala de tempo
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Rotaciona as coordenadas retangulares segundo o Ângulo dada
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Calcula valores harmônicos para servir de fator de correção para os planetas 
// Júpiter até Plutão, para melhorar a precisão dos cálculos
///////////////////////////////////////////////////////////////////////////////////////////////
...
/***********************************************************************************************************************/
/*	Rotinas para Cálculos com Efemérides
/***********************************************************************************************************************/

///////////////////////////////////////////////////////////////////////////////////////////////
// Solução da equação de Kepler
// t   -> Anomalia média em Graus
// ex  -> Excentricidade da órbita
// err -> Erro máximo em graus
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Variações de Módulo: Normaliza o ângulo para o intervalo [0,360]
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Sendo a e b interno ao círculo de 360 graus, retorna no intervalo [-180, +180]
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Computa as posições de Sol até Plutão, Quiron, os quatro asteóides e os nodos ( médio ou verdadeiro )
...
void CCalcMapa::ComputarPlanetasEfemerides( double dEscTempo )
{
	...
	for( i = oSun; i <= oLil; i++ ) 
	{
		...
		if( ComputarPosicaoPlanetas( i, dEscTempo*36525.0+2415020.0, ma_nObjetoCentral != oEar, &r1, &r2, &r3, &r4 ) ) 
		{
			...
			//Calcula coordenadas x,y,z a partir do azimute, altitude e distância
	...
	// Se for Heliocêntrico, move objeto Terra para índice Zero
	...
	// Se outro corpo central, reposiciona todas posições
	for( i = 0; i <= oLil; i++ ) 
	{
...
// Dado o índice de um objeto, o Dia Juliano, calcula a posição e a declinação zodiacal
// ( Latitude e Longitude planetária ), sua velocidade e distância a aprtir da Terra ou Sol
BOOL CCalcMapa::ComputarPosicaoPlanetas( int ind, double jd, int helio, double* obj, double* objalt, double* dir, double* space )
{
	...
	if( ind <= oPlu )      // Conversão de índices
	{
		iobj = ind-1;
	}
	else if( ind == oChi )
...
double CCalcMapa::LeDiferencTempo( double jd_ad )
{
	...
	{
		// delta tempo de 1637 até 2000, como tabulado no AE. Os valores entre 1629-1636 não foi tomado porque 
		// se ajustam muito mal à parábola 25,5t*t para o próximo intervalo.
		// O melhor ponto de cruzamento para trocar de parábola é 1637, onde os valore foram ajustados para a
		// continuidade
		6780, 6500, 6300,
		...
		5686, 5757, 5900, 5900, 6000, // AE 1993 e extraploção 
...
#if TIDAL_26									// Fórmula de Stephenson somente para 26" de termo da maré no movimento lunar
	...
	{ 
		// parábola, ajustado no valor[2000] 
		delta = 25.5 * cy * cy  - 25.5 * 4 + 63.00;
	} 
	else if( iyr >= 948 ) 
	{  
		// de 948 - 1637 utilizar parábola
		delta = 25.5 * cy * cy;
	} 
	else 
	{  
		//anterior a 984 utilizar outra parábola 
	...
	{ 
		// parábola, ajustada no valor[2000] 
...
// nPlaneta			Índice do planetas definida em Defines.h
//					nPlanetq == -1 calcula somente nut e ecl 
// dJDEfem;         Dia Juliano relativo à efeméride
// nFlag;			Flags 
// *pdLong			Longitude elíptica em graus
// *pdRaio			Raio em U.A,
// *pdLat =			La=titude eclíptica em graus
...
{
	// Tempo para os quais os dados estão sendo calculados
	struct sDataAtual  
	{
	...
	// PosHeliocentricaUp() verifica se as rotinas já foram chamadas para a mesma data
	...
	// Computação da Terra necessária se:
	// Lua ou Terra heliocêntrias
	// Qualquer planeta no geocêntrico exceto Lua e Lilith
	...
	switch( nPlaneta) 
	{
		...
		case MOON:
		{
			...
			luaData.lng = *pdLong;		// luaData será utilizada se TRUE_NODE 
			luaData.rad = *pdRaio;
			luaData.zet = *azet;
			*pdVeloc = 12;
			...
			*pdLat = fRADTODEG * asin( *azet / *pdRaio );
			...
			// Correção de velocidade da luz não aplicada a Lua e seus nodos
		...
		case MEAN_NODE:
		{
			*pdLong = smod8360( m_arrayEle[MOON].kn );
			...
			// A distância do nodo é o parâmetro orbital p = a ( 1-e^2);
			// A utilização do eixo como está sendo feita aqui não é correto, mas nunca é utilizada
			*pdRaio = pd[MOON].axis;
			*pdLat = 0.0;
			*pdVeloc = -0.053;
			break;
		...
		case TRUE_NODE: 
		{
			...
			*pdLong = smod8360( ( knv + knn ) / 2 );
			...
			// Veja comentário sobre a distância dos nodos
			*pdRaio = pd[MOON].axis;
			*pdLat = 0.0;
		...
		case LILITH: 
		{
			// Lilith ou Lua Negra ( dark Moon ) é ponto focal ( vazio ) da elipse lunar média
			// e eta a 180 graus do periélio.
			// Como a órbita lunar não se encontra na eclíptica, é necessário projetá-la sobre a eclíptica
			// do mesmo modo como são feitas para as órbitas planetárias 

			// O cômputo aqui é Lilith MÉDIO e não a verdadeira que poderia ser derivada de forma similar ao 
			// do Nodo Verdadeiro
			// Para o vetor raio de Lilith utiliza-se a fórmula simples:
			// Para maior precisão teria de ser levado em conta que o ponto focal da elípse não está
			// no centro da Terra mas sim no baricentro das posições da Terra e Lua
			// Para a velocidade será utilizada sempre a constante T do termo do periélio lunar
			...
			*pdRaio = 2 * pd[MOON].axis * e->ex;
			...
			// Para testar o erro Gravalaines, retornar a longitude não projetada em  pdLat.
			...
			//*pdLat = fRADTODEG * ASIN8( SINDEG( arg_lat) * SINDEG( e->in));
			//#else
			//*pdLat = NormalizaDeg( arg_lat + e->kn); // longitude não projetada, sem nutação
			//#endif
...
//
// PosHeliocentricaUp()
// Prepara os elementos de órbita e as perturbações para os planetas interno e a Lua
// PosHeliocentricaUp( t) é chamada por PosHeliocentrica() e pelo CalculoPrincipal().
// Esta rotina retorna o seu resultado em variáveis globais
// e grava a data para a qual foi calculada de modo que não é chamada duas vezes para a mesma data
void CCalcMapa::PosHeliocentricaUp( double jd_ad )
{
	...
	if( m_thelup == jd_ad )
	{
		// Se já foi calculada para esta data
		return; 
	}
	...
	for( i = SUN; i <= MARS; i++, pDadoEle++, pEle++ ) 
	{
		...
		ti = pEle->tj = td / 36525.0;	// Séculos julianos a partir da época
		ti2 = ti * ti;
		...
		// Para a Lua também lg1 *td será exatamente 10e-8 dentro de 5000 anos 
		...
		if( i == MOON ) 
		{
			// Calcula a eclíptica de acordo com Newcomb , APAE VI e a nutação de acordo com Exp. Suppl, 1961
			// idêntico ao elemnut() de Mark Pottenger
			// Todos os termos <= 0.01"
			// A teoria de nutação da IAU 1984 nãi implementada devido a utilização de outros elementos da Lua e do Sol

			double mnode  = fDEGTORAD * pEle->kn;			// Nodo lunar médio
			double mlong2 = fDEGTORAD * 2.0 * pEle->lg;		// 2 x a longitude lunar média
			double mg     = fDEGTORAD * pEle->ma;			// Anomalia lunar média
			double slong2 = fDEGTORAD * 2.0 * pEleTerra->lg;// 2 x longitude solar média( L)
			double sg     = fDEGTORAD * pEleTerra->ma;		// Anomalia solar média
			double d2     = mlong2 - slong2;				// 2 x a elongação da lua a partir do sol
	...
	// argumentos sa[] para os termos da perturbações
	...
	// Correção na perturbações de Júpiter de acorco com Pottenger, somente .03" e não esclarecido
...
// Computa as posições de todos os objetos( planetas em astrologia ) menos a Lua. 
// Os cálculos dos planetas externos a partir de Júpiter,
// incluindo Chiron e feita pela rotina OuterPosHeliocentrica
// PosHeliocentrica() calcula a posição verdadeira em relação a eclíptica e data dos equinócios
// Nutações não são consideradas e por isso devem ser consiredas pela rotina chamadora
// A latitude do Sol ( máxima de 0,5"segundos de arco ) é considerada como zero

// nPlaneta			Índice do planetas definida em Defines.h
// dJDEfem;         Dia Juliano relativo à efeméride
// *pdLon		Longitude em graus
// *pdRaio		Raio em U.A,
// *pdDist		Distância até a eclíptica em U.A.
...
BOOL CCalcMapa::PosHeliocentrica( int nPlaneta, double dJDEfem, double* pdLon, double* pdRaio, double* pdDist, double* pdVelLon, double* pdVelRaio, double* pdCoorZ )
{
	...
	if( nPlaneta == EARTH )  
	{
		// Alguns termos de período longo na longitude
	...
	if( nPlaneta == MARS )  
	{
		// Alguns termos de período longo 
	...
	*pdRaio = pEleData->axis * h1;
	...
	{
		dAnomVerdadeira = u; // Muito próximo do afélio
	}
	else
	{
		// Anomalia verdadeira
	...
	*pdDist = *pdRaio * sinv * sini;
	...
	if( fabs( PI - u ) < TANERRLIMIT )
	{
		vp = up / esquare;  // Velociade no afélio
	}
	else
	{
...
// *dRadio			Raio em Unidade astrnômica
// dCorrecLon		Correção na longitude em Segundos de arco, pode ser chamada dCorrecLon rk <> 0, mas não retorna valor
// rk,				Correção no raio na unidade  nona casa do log de r 
// dAnomaliaMedia	Anomaliza média do planeta man;	mean anomaly of planet 
...
// Computa as posições de Júpiter, Saturno, Urano, Netuno e Plutão e Chiron
// pela leitura das efemérides e pela aplicação da Interpolação 
// O Erro de interpolação é menor que 0.01" segundos de arco
// dJDEfem;  // Dia Juliano relativo à efeméride
BOOL CCalcMapa::OuterPosHeliocentrica( int nPlaneta, double dJDEfem, double* al, double* ar, double* az, double* alp, double* arp, double* azp )
{
	...
	else if( nPlaneta >= DEMETER && nPlaneta <= VESTA ) 
	{
		...
		if( dFlagJ0Asteroides != j0 )   // Lê dados dos quatros asteróides para as 6 etapas de cálculo
		{
			for( n = 0; n < 6; n++ ) 
			{
				jd = j0 + ( n - 2 ) * m_nPassoEfemeride;
	...
	{  
		...
		if( dFlagJ0Externos != j0 )   // Lê dados de todos os 5 planetas para as seis etapas
		{
			for( n = 0; n < 6; n++ ) 
			{
				jd = j0 + ( n - 2 ) * m_nPassoEfemeride;
...
// Interpolação de Everett, realizada após a de Pottenger ( versão 9 de Jul 1988 por Alois Treindl
// 
// nIndex		Interpolar entre x[nIndex] e x[nIndex-1], argument nIndex+p 
// nOrdem		ordem da interpolação, máxima 5 
// dArgInterp	Arqumento da interpolação
// x[],			array ocom os valores, necessário existir x[nIndex-nOrdem]..x[nIndex+nOrdem] 
...
void CCalcMapa::Interpolar( int nIndex, int nOrdem, double dArgInterp, double x[], double*axu, double* adxu )
{
	...
	// Interpolação de Everett para ordem 3
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Rotina que obtém dados da efeméride dos planetas JUPITER, SATURN, URANUS, NEPTUNE e PLUTÃO
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL CCalcMapa::PegaPosEfemeridePlanetasExternos( double dNumDiasJuliano )
{
	...
	{
		AfxMessageBox( "Erro na leitura da efeméride Externos.Efe" );
	}
	END_CATCH
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Rotina que obtém dados da efeméride dos asteróides DEMETER, PALLAS, JUNO e VESTA
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL CCalcMapa::PegaPosEfemerideAsteroides( double dNumDiasJuliano )
{
	...
	CATCH( CFileException, e )
	{
		AfxMessageBox( "Erro na leitura da efeméride Asteroides.Efe" );
	}
	END_CATCH
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Rotina que obtém dados da efeméride de CHIRON
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL CCalcMapa::PegaPosEfemerideChiron( double dNumDiasJuliano )
{
	...
	CATCH( CFileException, e )
	{
		AfxMessageBox( "Erro na leitura da efeméride Chiron.Efe" );
	}
	END_CATCH
...
void CCalcMapa::CalculoObjetoLua( double* al, double* ar, double* az )  
{
	...
	t = pEle->tj * 36525;  // dias  da época = 1900

	// Para novo formato, parâmetros com rotação a 360 graus
	...
	// Com correções periódicas e seculares
	...
	for( i = 0, mp = m45; i < NUM_MOON_CORR; i++, mp++ ) 
	{
		...
		// Aplicando as correções devido às mudanças nas constantes
		// Correções somente nos termos l' ( i1 ) e F ( i2 ), não na l ( i0 ), porque é menor 1ue 0.05 segundos
		// Não aplica correção de paralaxe ( seria para o cos( arg )
	...
	#if MOON_TEST_CORR  // Opcional, mais correções
	...
	// Cômputo dos termos planetários em longitude
	// Tomar todos > 0.5" e desprezar os termos seculares nos argumentos. O erros de fase
	// será menor que 10 graus após 3000 anos
	...
	*al = smod8360( pEle->lg + ( dlm + lk + lk1 + dlid ) / 3600.0 );
	...
	// novo gamma1C de 29 Jul 88, todos os termos > 0.4 " na tabela III, código 2 
	g1c += -0.725 * cos(           d );
	g1c +=  0.601 * cos(       2 * d );
	g1c +=  0.394 * cos(       3 * d );
	...
	// Desprezar termos planetários na latitude, código 4 na tabela III
	sinp = ( sinp + 3422.451 );
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Conversão de Heliocêntrica para geocêntrica
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CCalcMapa::ListaTeste()
{
	...
	osz.Format ( "Meio do Céu: %20.15lf\n", m_dMC );
	oszBuffer += osz;

	osz.Format ( "Ascendente : %20.15lf\n", m_dAsc );
	oszBuffer += osz;
	
	osz.Format ( "Vértice    : %20.15lf\n", m_dPosVertice );
	oszBuffer += osz;

	
	oszBuffer += "-------------------------------------------------------------------\n";
	oszBuffer += "Ângulo das cúspides\n";
	oszBuffer += "-------------------------------------------------------------------\n";
	...
	oszBuffer += "\n\n-------------------------------------------------------------------\n";
	oszBuffer += "Posição zodiacal dos objetos ( Ecliptico )\n";
	oszBuffer += "-------------------------------------------------------------------\n";
	...
	oszBuffer += "\n\n-------------------------------------------------------------------------------\n";
	oszBuffer += "Posição zodiacal dos objetos ( Equatorial )\n";
	oszBuffer += "-------------------------------------------------------------------------------\n";
	...
	oszBuffer += "Objetos\n";
	oszBuffer += "-------------------------------------------------------------------------------\n";
	oszBuffer += "Objeto          Ascensão Reta             Declinação              Latitude\n\n";
	...
	oszBuffer += "\n\n-------------------------------------------------------------------------------\n";
	oszBuffer += "Velocidade de Retrogradação\n";
	oszBuffer += "-------------------------------------------------------------------------------\n";
	oszBuffer += "                                    G  M \n";
	for( i=0; i < 31; i++ )
	{		
...
// Fix.
// Método estático (não membro da classe) que é chamado no constructor de CTgMetaAngulo e
// pode ser chamado por qualquer outro módulo do programa sem ser necessário criar um objeto desta classe.
...
double CCalcMapa::Fix( double dValor, int nPrecis )
{
	...
	// Se for menor que a precisão de double estabelece zero.
	...
	// Guarda a potência de 10 necessária aos cálculos.

	double dPot10 = pow( 10, nPrecis );

	// Se o valor é zero ou muito próximo retorna Ø.
	...
	// Multiplica o valor pela potência de dez da precisão e soma 0.501 para arredondar a parte fracionária.
	// floor retorna um double que representa o maior inteiro que é menor ou igual ao seu argumento.
